...
public class BORepositoryImportExport extends BORepositoryServiceApplication
						...
						public void accept(String t) {
							...
							for (String namesapce : namespaces) {
								if (namesapce != null && !namesapce.isEmpty()) {
									Logger.log(LoggingLevel.INFO, "import export: load [%s]'s classes.", namesapce);
	...
	public OperationResult<String> importData(FileItem data, emDataUpdateMethod updateMethod, String token) {
		...
		try {
			...
			Logger.log(LoggingLevel.DEBUG, MSG_TRANSFORMER_IMPORT_DATA, transformer.getClass().getName());
			// 转换文件数据到业务对象
			transformer.setInputData(new File(data.getPath()));
			transformer.transform();
			Logger.log(LoggingLevel.DEBUG, MSG_TRANSFORMER_IMPORT_DATA_SIZE, transformer.getOutputData().size());
				...
				for (int j = 0; j < transformer.getOutputData().size(); j++) {
					...
					try {
						...
						if (MyConfiguration.isDebugMode()) {
							...
							Logger.log(LoggingLevel.DEBUG, stringBuilder.toString());
						}
						// 导入的数据，源标记为I
	...
	public OperationResult<FileItem> exportData(DataExportInfo info, String token) {
		...
		try {
			...
			Logger.log(LoggingLevel.DEBUG, MSG_TRANSFORMER_EXPORT_DATA, transformer.getClass().getName());
			if (transformer instanceof ITransformerFile) {
				...
				Logger.log(LoggingLevel.DEBUG, MSG_TRANSFORMER_EXPORT_DATA_TO_FILE, file.getPath());
